...
@threestudio.register("stable-diffusion-apfo-guidance")
class StableDiffusionAPFOGuidance(BaseModule):
    @dataclass
    ...
    def configure(self) -> None:
        ...
        self.scheduler_lora = EulerDiscreteScheduler.from_pretrained(
            self.cfg.pretrained_model_name_or_path_lora,
            subfolder="scheduler",
            torch_dtype=self.weights_dtype,
        )
        ...
        self.num_views_per_step = self.cfg.num_views_per_step
        self.num_train_timesteps = self.scheduler.config.num_train_timesteps

        self.scheduler.set_timesteps(self.num_train_timesteps, device=self.device)
        self.scheduler_lora.set_timesteps(self.num_train_timesteps, device=self.device)
        self.timesteps = self.scheduler.timesteps

        self.min_step_percent = self.cfg.min_step_percent
        self.max_step_percent = self.cfg.max_step_percent

        self.set_min_max_steps(self.min_step_percent, self.max_step_percent)  # set to default value
        assert self.cfg.total_steps == self.num_views_per_step * (self.max_step - self.min_step), "Total steps does not match the multiplication of num_views_per_step and (max_step - min_step)."
        
        self.current_step = self.max_step - 1

        self.sigmas: Float[Tensor, "..."] = self.scheduler.sigmas.to(self.device)
    ...
    def set_min_max_steps(self, min_step_percent=0.2, max_step_percent=1.0):
    ...
    def train_lora(
        ...
        latents_model_input = self.scheduler_lora.scale_model_input(latents_noisy, t)
        ...
        pred_original_lora = self.scheduler_lora.step(noise_pred, t, latents_noisy).pred_original_sample
        self.scheduler_lora._step_index = None

        return F.mse_loss(pred_original_lora.float(), latents.float(), reduction="mean")
    ...
    def forward(
        ...
        **kwargs,
        ...
        t = torch.full((batch_size,), self.current_step).to(self.device)
        t_prev = torch.full((batch_size,), self.current_step - 1).to(self.device)
        t_prev = t_prev.clamp(0, self.max_step)
        ...
        loss_apfo = 0.5 * F.mse_loss(latents, target, reduction="sum") / batch_size
    ...
    def update_step(self, epoch: int, global_step: int, on_load_weights: bool = False):
        ...
        if (global_step + 1) % self.num_views_per_step == 0:
            self.current_step -= 1
